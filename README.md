...
> Note: this is a local MCP server. By default it runs as a stdio subprocess on the developer's or partner's machine and authenticates to a QuickBooks Online company. It can also serve MCP over streamable HTTP — run `npm run start:http` (listens on loopback only by default — `HOST`, default `127.0.0.1` — on `PORT`/`MCP_HTTP_PORT`, default `3000`, at `/mcp`) — for driving it from an HTTP-based MCP client instead of a subprocess. Every request to `/mcp` requires a bearer token from this server's own OAuth endpoints, which federate employee login to Intuit (set `MCP_OAUTH_REDIRECT_URIS` — see `.env.example`); leave it on loopback until it's actually reachable from claude.ai, and if you do bind it beyond loopback, also widen `MCP_HTTP_ALLOWED_HOSTS` (comma-separated hostnames; defaults to `127.0.0.1,localhost,::1`) to match, since requests with an unrecognized `Host` header are rejected as a DNS-rebinding defense. The stdio entry point (`npm start` / `node dist/index.js`) is unaffected either way.
